...
export function normalizeEnrollmentRow(row) {
  ...
  return {
    ...
    accessDisplayStatus: row.accessDisplayStatus ?? row.access_display_status ?? null,
    accessDisplayLabel: row.accessDisplayLabel ?? row.access_display_label ?? null,
    is_finished: row.is_finished === true || row.isFinished === true,
    finished_at: row.finished_at ?? row.finishedAt ?? row.courseFinishedAt ?? null,
