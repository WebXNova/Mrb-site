...
const TEACHER_QA_NAMESPACE = 'teacher-qa';

/** Pre-multer MIME gate only — acceptance uses magic bytes + parse. */
const storage = multer.diskStorage({
  destination(_req, _file, cb) {
    try {
      const dir = getQaAudioUploadDir(TEACHER_QA_NAMESPACE);
...
export const postTeacherAnswerRecording = [
  handleUpload,
  asyncHandler(async (req, res) => {
    await ensureQaAudioUploadDir(TEACHER_QA_NAMESPACE);
    ...
    try {
      const result = await finalizeQaAudioUpload(req, {
        namespace: TEACHER_QA_NAMESPACE,
      ...
      console.error('[teacher-qa-audio-upload] unexpected failure', error?.message || error);
