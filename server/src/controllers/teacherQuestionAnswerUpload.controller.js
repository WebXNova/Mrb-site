...
const storage = multer.diskStorage({
  destination(_req, _file, cb) {
    try {
      const dir = getQaImageUploadDir(TEACHER_QA_NAMESPACE);
...
export const postTeacherAnswerAttachment = [
  handleUpload,
  asyncHandler(async (req, res) => {
    await ensureQaImageUploadDir(TEACHER_QA_NAMESPACE);
    ...
    try {
      ...
      console.error('[teacher-qa-upload] unexpected failure', error?.message || error);
