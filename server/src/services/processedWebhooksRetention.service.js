/**
 * processed_webhooks retention — bounded batch deletes for rows older than retention window.
 */

import { mysqlPool } from '../config/mysql.js';
import { getProcessedWebhooksRetentionConfig } from '../config/processedWebhooksRetention.config.js';
import { recordProcessedWebhooksRetentionRun } from '../observability/processedWebhooksRetentionMetrics.service.js';
import { StructuredLogger } from '../utils/requestId.js';
import { runBatchedRetentionDeletes } from './retentionBatchRunner.js';

const auditLogger = new StructuredLogger({ service: 'processedWebhooksRetention' });
const LOG_PREFIX = '[processed-webhooks-retention]';
 ...
 * @param {number} [nowMs]
 */
export function computeProcessedWebhooksRetentionCutoff(retentionDays, nowMs = Date.now()) {
 ...
 * @param {Date} cutoff
 */
export async function countExpiredProcessedWebhooks(pool, cutoff) {
 ...
 */
export async function deleteExpiredProcessedWebhooksBatch(pool, { cutoff, batchSize }) {
 ...
 * }} [opts]
 */
export async function runProcessedWebhooksRetention(opts = {}) {
  ...
  const cutoff = computeProcessedWebhooksRetentionCutoff(retentionDays, opts.nowMs);
  ...
  summary.expiredBeforeRun = await countExpiredProcessedWebhooks(pool, cutoff);

  if (dryRun) {
    summary.remainingExpired = summary.expiredBeforeRun;
    summary.durationMs = Date.now() - started;
    recordProcessedWebhooksRetentionRun(summary);
    auditLogger.info('Processed webhooks retention dry-run', {
  ...
  const batchResult = await runBatchedRetentionDeletes({
    deleteBatch: (limit) => deleteExpiredProcessedWebhooksBatch(pool, { cutoff, batchSize: limit }),
    batchSize,
    batchPauseMs,
    maxBatchesPerRun,
    maxRetriesPerBatch,
  ...
  summary.remainingExpired = await countExpiredProcessedWebhooks(pool, cutoff);
  ...
  auditLogger.info('Processed webhooks retention completed', {
