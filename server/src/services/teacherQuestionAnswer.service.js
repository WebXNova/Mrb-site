...
} from './teacherQuestionDetail.service.js';
import { upsertTeacherAnswerInTransaction } from './teacherAnswer.service.js';
...
/**
 * Submit teacher answer with ownership enforced in SQL transaction.
 * @param {number} teacherId
 * @param {string|number} questionId
 * @param {{ body: string, imageUrl?: string|null, audioUrl?: string|null }} payload
 */
export async function submitTeacherQuestionAnswer(teacherId, questionId, payload) {
  ...
  try {
    ...
    await upsertTeacherAnswerInTransaction(connection, {
      questionId: id,
      teacherId: tid,
    ...
    if (!detail) {
      throw new ApiError(500, 'Answer was saved but could not be loaded', { code: 'ANSWER_LOAD_FAILED' });
    }
