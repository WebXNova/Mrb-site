...
import { TEST_ACCESS_TYPE_VALUES } from '../constants/testAccessType.constants.js';
import { isStandaloneAccessType } from './testAccessType.js';
...
export const testBasicInfoBodySchema = z
  .object({
    course_id: z.preprocess(
      ...
      z
        .union([
          z.null(),
          z.coerce.number({ invalid_type_error: 'course_id must be an integer' }).int().positive(),
        ])
        .optional()
    ),
    test_access_type: z.preprocess(
      (value) => {
        if (value == null || value === '') return 'course_locked';
        return String(value).trim().toLowerCase();
      },
      z.enum(TEST_ACCESS_TYPE_VALUES, {
        errorMap: () => ({
  ...
  .superRefine((data, ctx) => {
    if (data.test_access_type === 'course_locked') {
    ...
    } else if (isStandaloneAccessType(data.test_access_type) && data.course_id != null) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
